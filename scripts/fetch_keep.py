...
class KeepClient:
    def __init__(self) -> None:
        self.session, self.headers = self._build_session()

    def _build_session(self) -> tuple[requests.Session, dict[str, str]]:
        if not KEEP_TOKEN and not (KEEP_MOBILE and KEEP_PASSWORD):
            log.err("缺少凭证：KEEP_TOKEN 或 KEEP_MOBILE + KEEP_PASSWORD")
            sys.exit(1)
        ...
        try:
            ...
            if response.status_code != 200:
                log.err(f"登录失败: {response.text[:120]}")
                sys.exit(1)
        ...
        *,
        ...
        for sport_type in KEEP_SYNC_TYPES:
            if sport_type not in VALID_SPORT_TYPES:
                log.warn(f"忽略未知类型: {sport_type}")
                continue
            all_summaries.extend(
                self._fetch_stats_pages(
                    sport_type,
                    known_ids=known,
                    cutoff_ts=cutoff_ts,
    ...
    def sync_records(
        ...
        pending = self._filter_pending(local_data, summaries)
        ...
        def label(pair: tuple[str, dict]) -> str:
            sport_type, stats = pair
            safe_time = keep_time_to_local_str(stats.get("doneDate"))
            keep_id = extract_keep_id(stats.get("id") or "")
            fallback = f"keep_{keep_id}" if keep_id else "?"
            return f"{sport_type} {safe_time or fallback}"

        def build(pair: tuple[str, dict]) -> dict | None:
            sport_type, stats = pair
            detail = self._fetch_run_detail(sport_type, stats.get("id") or "")
            return format_keep_record(sport_type, stats, detail)
    ...
    def _fetch_stats_pages(
        ...
        *,
        ...
        while True:
            ...
            log.debug(f"列表 {sport_type} lastDate={last_date}")
            payload_root = self._get_json(
                url,
            ...
            if max_pending and pending_count() >= max_pending:
                log.debug(f"{sport_type} 待同步已达 {max_pending}，停止翻页")
                break
            time.sleep(0.5)

        log.info(f"{sport_type} 摘要 {len(collected)} 条（{pages} 页）")
        return collected

    def _get_json(
        self, url: str, *, error_label: str, use_err: bool = False
    ) -> dict | None:
        log_fail = log.err if use_err else log.warn
        try:
            response = request_with_retries(
                self.session,
                "GET",
    ...
    def _fetch_run_detail(self, sport_type: str, stats_id: str) -> dict | None:
        keep_id = extract_keep_id(stats_id)
        if not keep_id:
            return None
        url = RUN_LOG_API.format(sport_type=sport_type, run_id=stats_id)
        return self._get_json(url, error_label=f"详情失败 {keep_id}")
    ...
    def _filter_pending(
        ...
        cutoff_ts = cutoff_timestamp(local_data)
        summaries.sort(
            key=lambda pair: parse_time(
                keep_time_to_local_str(pair[1].get("doneDate")) or ""
            )
        )
        return [
            (sport_type, stats)
...
def main() -> None:
    set_log_scope("Keep")
    log.section("Keep 运动同步")
    log_sync_startup(PATHS)
    if not _HAS_CRYPTO or not _HAS_POLYLINE:
        log.info("未装 pycryptodome/polyline，轨迹折线将为空")
    ...
    client = KeepClient()
    summaries = client.fetch_all_summaries(local_data, sync_limit=SYNC_LIMIT)
    log.info(f"摘要合计 {len(summaries)} 条")

    final_data, fetched_count, added_count = client.sync_records(local_data, summaries)
    finalize_sync(
        PATHS,
        local_data,
        final_data,
