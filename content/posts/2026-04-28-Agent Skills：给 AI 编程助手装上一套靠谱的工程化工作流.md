...
它的问题不是"生成不出来"，而是不按靠谱的软件工程流程做事：一上来就写代码，不先澄清需求；改了一大坨也不拆分；不写测试，修完也没法证明真的修好了；不会主动做 review、安全检查、性能检查；上线前更没有灰度和回滚意识。
...
- 明显提升 AI 产出的下限——少跑偏、少跳步骤、少无测试改动、少无边界感的重构
- 结构清晰、迁移性强——不绑定单一工具，是一个可复用的规则层
- 很适合做团队共识——底层模型可以换，但流程和纪律可以沉淀

**局限也要看清：**

- 它不是装完就自动变强——你得正确接入工具、选对技能、在关键时刻明确调用
...
1. clone 仓库
2. 先挑上面说的 3 个核心 skill
3. 接入最常用的工具（Claude Code 用 slash command、Cursor 放 `.cursor/rules/`、Gemini CLI 装 native skills）
4. 连续用一周，观察三个指标：AI 是否更少跑偏、返工是否减少、是否更敢 merge AI 写的代码

三个指标有改善，这套东西就值。

## 最后

AI 编程的关键问题已经不是"模型够不够聪明"，而是有没有一套好的工作流约束。

一个没有工程纪律的强模型照样会制造混乱。而一个有 spec 意识、测试意识、review 习惯、上线流程的 AI，即使模型本身没那么神，也更容易成为真正可靠的开发伙伴。
