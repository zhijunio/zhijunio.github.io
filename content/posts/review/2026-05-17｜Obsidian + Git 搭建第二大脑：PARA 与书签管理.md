...
> **2026-05 后更新**：逻辑日推已并入**概念日推**（`030-digest/concept/`，逻辑域）；`030-digest/logic/` 与周六「逻辑+概念自测」已取消。当前为工作日 **8 路日推** + 周日工作周报，概念日推 **07:00**。下文 cron 表与目录树已按现行配置修订；叙事段落仍反映当周建设过程。

> **2026-06 架构更新**（现行见 [README.md](https://github.com/zhijunio/second-brain/blob/main/README.md)）：`0xx` 输入 · `1xx` 人生 · `2xx` 知识域（`200-java/`、`210-ai/`）· `3xx` 输出（`300-blog/tech/`、`300-blog/review/`）· `_system/` Hermes。内部工作周报在 `030-digest/weekly/`；对外周回顾在 `300-blog/review/`（博客仓 `content/posts/review/`）。

## 本周摘要

- **知识库**：Obsidian Vault 命名为 second-brain，Git 同步；目录按「输入 → 内化 → 输出」流转（`0xx` → `1xx` / `2xx` → `300-blog/`）；Java 深研在 `200-java/topic/`，职场上下文在 `100-work/`。
- **入口**：Telegram + Hermes 替代部分自建服务；书签用 Skill 写入 Markdown，Memos 作 Web 展示。
- **自动化**：**8 项工作日日推 + 1 项周日工作周报**，产出进 `030-digest/`、Memos 与 Telegram；推送为待消化输入，重在联系自身与运用（见 `_system/cron/README.md`）。
...
**保存**：`020-bookmark/YYYY-MM-DD-{文件名段}.md`。文件名段与 `[锚文]` **字面一致**，仅把 **`/`** 换成 **`-`**；**不**
...
│   ├── 010-raw/
│   ├── 020-bookmark/
│   └── 030-digest/             # 日推；weekly/ 为内部工作周报
│       ├── biography/ car/ concept/ cooking/
│       ├── github-repo/ interview/ parenting/ relationship/
│       └── weekly/
├── 1xx 人生领域
│   ├── 100-work/               # career、project（不含 Java 专题）
│   ├── 110-growth/
│   ├── 120-family/
│   ├── 130-health/
│   ├── 140-finance/
│   └── 150-interest/
├── 2xx 知识域
│   ├── 200-java/               # topic/、wiki/、assets/
│   └── 210-ai/                 # prompt/、assets/
├── 3xx 输出
│   └── 300-blog/               # tech/、review/ → 博客仓 content/posts/
└── _system/                    # cron/、skill/
```

**命名约定：**

- `0xx` 输入 · `1xx` 人生 · `2xx` 知识域 · `3xx` 输出 · `_system` 运行。
...
- `000-todo/`：待办、计划、方案草稿、想法与行动提醒。
- `010-raw/`：外部收藏原文，只保存，不提炼。
- `020-bookmark/`：URL 书签一行摘要；待决定是否深读，**不是**日推 digest。
- `030-digest/`：Hermes 日推等短摘要；待消费，不代表已内化。
- `030-digest/weekly/`：Hermes **内部**工作周报 `YYYY-Www.md`。

### 内化层（1xx · 2xx）

- `100-work/`：职场上下文（career、project、assets）。**不含** Java 专题。
- `110-growth/`～`150-interest/`：人生领域长期内容。
- `200-java/topic/`：Java 专题、5D 拆解（`reading/`）、`wiki/`。
- `210-ai/prompt/`：AI 提示词与工具参考。

### 输出层（3xx）

- `300-blog/tech/`：可公开发布的技术长文。
- `300-blog/review/`：对外**周回顾**（博客仓 `content/posts/review/`；与 `030-digest/weekly/` 区分）。

### 系统层

- `_system/cron/`：定时任务提示词。
- `_system/skill/`：按需技能（书签入库、技术文章拆解等）。
```

**使用规则：**

1. 原始收藏全文 → `010-raw/`；书签一行摘要 → `020-bookmark/`。
2. Hermes 日推 → `030-digest/`；**内部**工作周报 → `030-digest/weekly/`。
3. 待办、计划 → `000-todo/`。
4. 职场上下文 → `100-work/`；Java 深研 → `200-java/topic/`；AI 提示词 → `210-ai/prompt/`。
5. 可公开发布 → `300-blog/tech/` 或 `300-blog/review/`。
6. `_system/` 只放 Hermes 提示词，不放普通笔记。

**推荐的最小流转路径：**

```text
0xx 输入 → 1xx / 2xx 内化 → 300-blog（可选）→ 博客仓
```

**补充说明：**

- `030-digest/weekly/` 承接 Hermes **内部**工作周报；`300-blog/review/` 承接**对外**可发布周回顾。
- `1xx` / `2xx` 是消化后的长期沉淀位置。

## Hermes 定时任务

在我的 second-brain 中，`030-digest` 承接 Hermes/AI 的预处理输入。定时任务按 `_system/cron/` 提示词执行：推送是为了**看懂、联系自身、用起来**，落盘只是待消化输入（详见 `_system/cron/README.md`）。
...
| 时间 | 任务 | 落盘 |
|------|------|------|
| 06:00 | 育儿日推 | `030-digest/parenting/` |
| 07:00 | 概念日推 | `030-digest/concept/`（含逻辑域等 8 域） |
| 09:00 | 面试日推 | `030-digest/interview/` |
| 10:00 | Java 仓库日推 | `030-digest/github-repo/` |
| 11:00 | 传记日推 | `030-digest/biography/` |
| 12:00 | 汽车日推 | `030-digest/car/` |
| 17:00 | 做菜日推 | `030-digest/cooking/` |
| 19:00 | 亲密关系日推 | `030-digest/relationship/` |

**周末**

| 时间 | 任务 | 落盘 |
|------|------|------|
| 周日 06:00 | 工作周报 | `030-digest/weekly/YYYY-Www.md` |

工作周报统计**最近 7 天**（周一～周日），以 2026 年度目标为导向，做注意力审计与下周可执行建议。提示词见 `_system/cron/` 各同名文件。
...
- 你读了多少？（`200-java/topic/reading/` 里当时只有 1 篇深度拆解）
...
- second-brain 中保留 raw 原始收藏文件，删除旧 wiki 目录；Java 专题与 5D 拆解迁入 `200-java/topic/`；职场上下文留 `100-work/`；书签独立为 `020-bookmark/`；待办合并进 `000-todo/`；Hermes 内部工作周报 → `030-digest/weekly/`（`YYYY-Www.md`），对外周回顾 → `300-blog/review/`；cron/skill 统一在 `_system/`（调度见 `cron/README.md`）。

## 分享

有意思的输入多数会同步到 Telegram「[ZhiJun Share](https://t.me/s/zhijun_share)」。这里再挑一部分，方便当周回顾 / newsletter 用。
