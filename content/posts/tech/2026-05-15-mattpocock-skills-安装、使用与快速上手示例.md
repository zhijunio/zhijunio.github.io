...
- `to-prd`：把上下文收成 PRD，**发布到 issue tracker**（GitHub/GitLab Issue 正文，或本地 `.scratch/<feature>/PRD.md` 等）——永远不进 `docs/prd/*.md` 这类独立目录
- `to-issues`：把 PRD / 方案拆成可抓取的垂直切片 issue；会先列出草案**请你批准**，再创建

这些都不是“万能技能”，而是非常窄的工作流切片。

另外，每个 skill **做完就停**，不会自动链到下一个——要你在对话里显式再加载 `to-prd`、`tdd` 等。这和「一条超级 prompt 包办全流程」是刻意相反的设计。
...
它不是在生成业务代码，而是在初始化这套技能运行时依赖的仓库约定。典型产出包括：

- `docs/agents/issue-tracker.md` — issue 写 GitHub 还是本地 markdown
- `docs/agents/triage-labels.md` — 五个 triage 角色的 label 词汇
- `docs/agents/domain.md` — `CONTEXT.md`、`docs/adr/` 在哪、怎么读
- 根目录 `AGENTS.md` 或 `CLAUDE.md` 里的 `## Agent skills` 块，把上面三份文档指给后续 skill

它会先探索仓库、跟你确认，再写入——不是无脑套模板。

**依赖强弱也不一样：**

| 类型 | 技能 | 没有 setup 时 |
| --- | --- | --- |
| 硬依赖 | `to-prd`、`to-issues`、`triage` | 容易猜错 tracker / label |
| 软依赖 | `tdd`、`diagnose`、`zoom-out` 等 | 能跑，但领域词汇、ADR 引用会更糙 |
...
```mermaid
flowchart TD
    A["setup-matt-pocock-skills"] --> B["docs/agents + AGENTS.md"]
    B --> C["grill-me / grill-with-docs"]
    C --> D["需求与边界收敛"]
    D --> E["to-prd → Issue 上的 PRD"]
    E --> F["to-issues → 子 issue 列表"]
    F --> G["tdd / 实现"]
    G --> H["diagnose（若失败）"]
    H --> G
    G --> I["关 issue / 验收评论"]
...
- **交付一条需求**时，常见顺序是 `grill → to-prd → to-issues → tdd`，每一步结束都要你显式开下一步
- **入门练手**时，`grill-me → tdd → diagnose` 仍是一条稳定的最小工程闭环（见第八节）
...
- 用户说要 TDD
- 用户说要 debug
- 用户说要把方案收成 PRD 或拆成 issues
- 用户说要把模糊想法问透（`grill-me`）或对齐领域文档（`grill-with-docs`）
...
- `grill-me` — 通用对齐，不改仓库文档
- `grill-with-docs` — 写代码前更常用；可懒更新 `CONTEXT.md`、少量 ADR
- `to-prd` — 合成 PRD 并发布到 issue tracker
- `to-issues` — 垂直切片拆 ticket，先 quiz 再创建
- `triage` — 五角色状态机驱动 issue 流转
...
**已发布、日常可用（`engineering/` + `productivity/`，进官方 plugin bundle）：**

- `zoom-out`、`improve-codebase-architecture`
- `write-a-skill`、`handoff`、`caveman`
- `migrate-to-shoehorn`、`scaffold-exercises`（在 `misc/`）

**草稿或未 ship（`in-progress/` 等，别当默认可用）：**

- `teach`、`writing-beats`、`writing-fragments`、`writing-shape`

如果你是第一次接触，不需要把这些技能都背下来。  
真正最先该掌握的，通常只有 4 个：

- `setup-matt-pocock-skills`
- `grill-me`（做功能开发时往往换成 `grill-with-docs`）
- `tdd`
- `diagnose`

它们已经能覆盖最常见的工程起手式。要把想法真正交付出去，再加上 `to-prd` 和 `to-issues`（见第九节）。
...
`mattpocock/skills` 适合补局部动作，让某一步别做歪；每个 skill 显式 handoff，**不自动串联**。  
Superpowers 适合管全局节奏，让整条链路别跑偏，也更强调阶段之间的自动推进感。
...
装整个仓库时，**务必勾选 `setup-matt-pocock-skills`**——它是后面 issue / triage / 领域文档类 skill 的前置。
...
- `AGENTS.md` / `CLAUDE.md` 里的 `## Agent skills` 索引块

`domain.md` 描述 `CONTEXT.md`、`docs/adr/` 在哪、怎么读；**不一定会新建** `CONTEXT.md`——只有项目真的需要术语表时才值得加。

初始化时还会让你选 **issue tracker** 类型，这直接决定 `to-prd` 把 PRD 写到哪里：

| 类型 | PRD 落在哪 | 子 issue 落在哪 |
| --- | --- | --- |
| **GitHub** | 仓库 Issues 正文（`gh` CLI） | 同上 |
| **GitLab** | 仓库 Issues 正文（`glab` CLI） | 同上 |
| **Local markdown** | `.scratch/<feature-slug>/PRD.md` | `.scratch/<feature-slug>/issues/01-<slug>.md` … |
| **Other**（Jira、Linear 等） | 你描述的外部 tracker 里的一条 ticket | 按 `docs/agents/issue-tracker.md` 里记录的流程 |

Local markdown 适合无远程、单人项目；Jira/Linear 等在 setup 里选 **Other**，用一段话写清创建/更新 ticket 的步骤，agent 会照 `issue-tracker.md` 执行。
...
如果你第一次用这套仓库，我建议不要一上来把所有技能都掏出来。下面这条线练的是**四种工程动作**，不是完整交付流水线（完整闭环见第九节）。
...
| 澄清 | `grill-me` / `grill-with-docs` | 把“模糊想法”压成可实现边界；写代码前优先后者 |
...
要把一个想法真正推进到可追踪的交付，Matt 侧更常见的是这条线：

```text
1. /setup-matt-pocock-skills          # 一次性
2. /grill-with-docs                   # 对齐边界，必要时更新 CONTEXT / ADR
3. /to-prd                            # PRD 发布到 GitHub Issue（或本地 tracker）
4. /to-prd 完成后 → /to-issues       # 拆垂直切片，批准后再建子 issue
5. /tdd                               # 按切片实现
6. 实现完成 → 关子 issue，在 PRD issue 贴验收说明
```

自然语言写法同理，关键是**每一步结束都要你开口开下一步**。

抽象示例（GitHub 场景，数字仅示意流程）：

| 步骤 | 产物 |
| --- | --- |
| `to-prd` | Issue #11：PRD 正文（Problem / Solution / User Stories / 测试接缝） |
| `to-issues` | Issue #12–#15：每条一个 tracer-bullet 垂直切片（先列表征求你同意） |
| `tdd` + 合并 | 逐条关 issue；PRD issue 留最终验收评论 |

若 tracker 是 **Local markdown**，同一流程对应为：`.scratch/my-feature/PRD.md` → `.scratch/my-feature/issues/01-….md` 等；triage 状态写在各文件顶部的 `Status:` 行，而不是 GitHub label。

`to-prd` 会打 `ready-for-agent` label；`to-issues` 创建的实现 ticket 也会带约定好的 triage 词汇——这些都依赖第七节里的 setup。

## 十、和 SDD-Lite 等阶段技能并存

如果你同时在用 [SDD-Lite](https://github.com/zhijunio/sdd-lite) 一类 **plan → build → ship** 的阶段技能，两套可以分工，不必二选一：

| Matt | SDD-Lite（下游应用仓） |
| --- | --- |
| `to-prd` → Issue 上的 PRD | `sdd-plan` → `docs/sdd/plan/YYYY-MM-DD-*.md` |
| `to-issues` → 可抓取的 GitHub issue | 可对照 plan 里的 Test-first checklist |
| `grill-with-docs` → `CONTEXT.md` / ADR | 可选 `docs/sdd/CONTEXT.md`（仅术语表） |
| 维护者仓：issue 驱动现代化 | 下游仓：`sdd-build` / `sdd-ship` 闭环 |

经验法则：**Matt 管「想清楚 + 拆 ticket + TDD/排障」**；**SDD-Lite 管「增量计划契约 + 按 Acceptance 交付」**。维护工具链本身用 Matt 往往更顺手；产品功能增量用 SDD-Lite 往往更清晰。
