...
- **知识库**：Obsidian Vault 命名为 second-brain，Git 同步；目录按 PARA 思路重组（`01-raw` → `02-digest` → `11-note` / `12-journal` → 领域层）。
...
- `03-todo/`：待办事项和行动提醒。
- `04-plan/`：阶段计划、方案草稿、路线图。
- `05-idea/`：零散想法、灵感、问题意识。

### 输出层

- `11-note/`：不强绑定某个领域的通用笔记、总结、方法、思考。
- `12-journal/`：按时间组织的输出，如周报、复盘、阶段记录。
...
3. 自己的待办、计划、想法分别放到 `03-todo/`、`04-plan/`、`05-idea/`。
4. 自己消化后的输出，优先放到 `11-note/` 或 `12-journal/`。
...
```text
01-raw -> 02-digest -> 11-note / 12-journal -> 21-growth / 22-family / 23-work
```

**补充说明：**

- `03-todo`、`04-plan`、`05-idea` 主要承接输入。
- `02-digest` 承接 Hermes/AI 的预处理输入。
- `11-note` 和 `12-journal` 是你的主要输出层。
...
- 你读了多少？（02-resource/note/reading/ 里只有 1 个文件）
...
- second-brain 中保留 raw 原始收藏文件，删除 wiki 目录，增加输出目录。
