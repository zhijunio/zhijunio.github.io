...
| A | 文章页 | 仅 `mermaid: true` 加载 Mermaid；正文含围栏代码才加载复制；有内联图才 Photosuite |
| B | `dist` | `_astro` 长期缓存；HTML 短期可再验证；`prune-dist-images` 减小部署体积 |
| C | 约定文档化 + 可选审计脚本 | 新能力默认不进入列表页 |

## 产品约束（已确认）

- **Mermaid**：仅在 frontmatter `mermaid: true` 时启用（remark 转换 + 客户端 `MermaidLoader`）。
...
- 移除 Photosuite、BackToTop、Umami（用户选择方案 E）。
